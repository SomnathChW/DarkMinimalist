...
interface AppContextType {
  ...
  handleNonImageExtensionToggle: (extension: string, selected: boolean) => void;
  handleRenameToggle: (shouldRename: boolean) => void;
  handleBaseNameChange: (name: string) => void;
  setResizeSettings: (settings: Partial<AppContextType["resizeSettings"]>) => void;
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [nonImageExtensions, setNonImageExtensions] = useState<FileExtension[]>([]);
  ...
  const processSelectedFiles = (files: File[]) => {
    ...
    setNonImageExtensions(
      Array.from(nonImageExts).map(ext => ({ extension: ext, selected: false }))
    );
  ...
  const nextImage = useCallback(() => {
    ...
    const currentIndexInSelected = selectedIndices.indexOf(currentImageIndex);
